...
describe("AgentExecutionRepository Integration Tests", () => {
  ...
  describe("markAsReviewed", () => {
    it("should mark execution as reviewed with correctness", async () => {
      await withTransaction(pool, async (client) => {
        const execution = await createTestExecution(client);
  ...
  describe("getExecution", () => {
    it("should return execution by ID", async () => {
      await withTransaction(pool, async (client) => {
        const created = await createTestExecution(client, {
  ...
  describe("queryExecutions", () => {
    it("should filter by phase", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, {
          phase: "CATEGORY_DISCOVERY",
        });
        await createTestExecution(client, {
          phase: "SCHEMA_GENERATION",
    ...
    it("should filter by status", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { status: "success" });
        await createTestExecution(client, { status: "error" });
        await createTestExecution(client, { status: "success" });
    ...
    it("should filter by model", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { llm_model: "gpt-4" });
        await createTestExecution(client, { llm_model: "claude-3" });
        await createTestExecution(client, { llm_model: "gpt-4" });
    ...
    it("should filter by review status", async () => {
      await withTransaction(pool, async (client) => {
        const reviewed = await createTestExecution(client, {
          is_reviewed: true,
        });
        await createTestExecution(client, { is_reviewed: false });
    ...
    it("should filter by product_id", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { product_ids: [100, 101] });
        await createTestExecution(client, { product_ids: [200] });
        await createTestExecution(client, { product_ids: [100, 300] });
        ...
        expect(executions.length).toBeGreaterThanOrEqual(2);
        expect(executions.every((e) => e.product_ids?.includes(100))).toBe(
          true,
        );
      });
    });
    ...
    it("should filter by category_id", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { category_ids: [5, 6] });
        await createTestExecution(client, { category_ids: [7] });
        await createTestExecution(client, { category_ids: [5, 8] });
  ...
  describe("getExecutionsByStatus", () => {
    it("should return executions with specific status", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { status: "partial" });
  ...
  describe("getExecutionsByModel", () => {
    it("should return executions for specific model", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { llm_model: "claude-3-opus" });

        const executions =
          await repository.getExecutionsByModel("claude-3-opus");
  ...
  describe("getUnreviewedExecutions", () => {
    it("should return only unreviewed executions", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { is_reviewed: false });
        await createTestExecution(client, { is_reviewed: true });
  ...
  describe("getExecutionsForProduct", () => {
    it("should return executions affecting a product", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { product_ids: [123, 456] });
        await createTestExecution(client, { product_ids: [789] });

        const executions = await repository.getExecutionsForProduct(123);

        expect(executions.length).toBeGreaterThanOrEqual(1);
        expect(executions.every((e) => e.product_ids?.includes(123))).toBe(
          true,
        );
      });
    });
  ...
  describe("getExecutionsForCategory", () => {
    it("should return executions affecting a category", async () => {
      await withTransaction(pool, async (client) => {
        await createTestExecution(client, { category_ids: [42, 43] });
        await createTestExecution(client, { category_ids: [99] });

        const executions = await repository.getExecutionsForCategory(42);

        expect(executions.length).toBeGreaterThanOrEqual(1);
        expect(executions.every((e) => e.category_ids?.includes(42))).toBe(
          true,
        );
      });
    });
