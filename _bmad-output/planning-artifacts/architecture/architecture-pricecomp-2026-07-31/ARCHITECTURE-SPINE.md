...
- **Rule:** A framework-free `contracts/v1` package owns `SourceIdentity`, `CatalogSnapshotRef`, `ProductSnapshot`, `Money`, `EvidenceRef`, typed trait/quantity values, command request/result schemas, stable domain error codes, and each stage input/outcome. `EvidenceRef` names the source field, the verbatim substring (`evidence_span`), and a SHA-256 hash of that field's text at extraction time — matching AD-4; byte-offset addressing, NFC normalization guarantees, and revision carry-forward/invalidation cascades are out of MVP (see §Deferred). Stage outcomes use `success`, `unknown`, `defer`, `invalid`, or `retryable_failure`, distinguish proposals from committed command receipts, carry prerequisite/produced revisions and immutable config IDs, and are the sole evaluator input. Under AD-16's single-writer runtime, taxonomy mutating commands do **not** carry a taxonomy-wide topology revision with compare-and-set; per-entity optimistic revision on authoritative tables (AD-23) suffices — topology CAS returns with the deferred concurrency machinery (see §Deferred). R/D/A workflow variants may change who produces or applies an outcome, never its schema or commit semantics.
