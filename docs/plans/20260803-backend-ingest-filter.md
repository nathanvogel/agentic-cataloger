...
Make `agentic-cataloger ingest --source-category "Milchprodukte"` work: same import path as today’s bulk ingest, but only upserting rows that match a filter. Empty filter stays bulk; a filter that happens to match one row is “single” — no mode branches, and no pin-by-id flag (use a distinctive `--keyword` when you want one row).

## Locked decisions

- **Filter is criteria, not a DB manifest.** Old BMAD ACs mentioned an “immutable ingest-selection manifest” table; the living roadmap demo and 4.5 (`agentic-cataloger run --source-category …`) re-specify the filter on the CLI. No new table in 1.5.
- **Match rules**
...
- **Apply inside** [`import_snapshot`](backend/src/agentic_cataloger/catalog/commands.py) so CLI, tests, and a future pipeline launcher share one path. Snapshot registration stays whole-file (checksum unchanged); the filter only narrows which observations upsert.
...
```mermaid
flowchart LR
  CLI["agentic-cataloger ingest --source-category"] --> Runner
...
Add [`backend/src/agentic_cataloger/catalog/ingest_filter.py`](backend/src/agentic_cataloger/catalog/ingest_filter.py):
...
In [`backend/src/agentic_cataloger/catalog/commands.py`](backend/src/agentic_cataloger/catalog/commands.py):
...
- [`cli.py`](backend/src/agentic_cataloger/platform/cli.py): add `--source-category` and `--keyword`; pass through to runner
- [`runner.py`](backend/src/agentic_cataloger/platform/ingest/runner.py): accept `ingest_filter: IngestFilter`, put it on each `ImportSnapshotRequest`; include matched/filtered counts in `IngestRunSummary` / stdout
...
- `agentic-cataloger run` / ProductRepository `list_matching` (4.5) — only leave `IngestFilter` importable from `catalog`
...
```bash
agentic-cataloger ingest --source-category "Milchprodukte" --retailer migros
# upserted ≈ rows whose source_category contains Milchprodukte; other products untouched
agentic-cataloger ingest   # empty filter still imports latest CSV(s) in full
```
